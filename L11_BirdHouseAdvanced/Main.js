...
(function (L11_BirdHouseAdvanced) {
    ...
    L11_BirdHouseAdvanced.handleload = handleload;
    function create() {
        for (let index = 0; index < 500; index++) {
            let randscale = Math.floor(Math.random() * (3 - 1) + 1);
            let snowflake = new L11_BirdHouseAdvanced.Snowflake(randscale);
            moveables.push(snowflake);
        }
        for (let indexbird = 0; indexbird < 10; indexbird++) {
            let randomscale = (Math.random() * (0.4 - 0.2) + 0.2);
            let bird = new L11_BirdHouseAdvanced.Bird(randomscale);
            moveables.push(bird);
        }
    }
    function update() {
    ...
    L11_BirdHouseAdvanced.drawBackground = drawBackground;
