namespace L11_BirdHouseAdvanced {
    ...
    function create(): void {
        ...
        for (let index: number = 0; index < 500; index++) {
            ...
            let randscale: number = Math.floor(Math.random() * (3 - 1) + 1);
            let snowflake: Snowflake = new Snowflake(randscale);
            moveables.push(snowflake);
        }

        for (let indexbird: number = 0; indexbird < 10; indexbird++) {
            let randomscale: number = (Math.random() * (0.4 - 0.2) + 0.2);
            let bird: Bird = new Bird(randomscale);
            moveables.push(bird);
        }
    }
